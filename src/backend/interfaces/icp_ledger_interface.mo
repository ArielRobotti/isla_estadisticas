...
module {
  ...
  public type FieldsDisplay = { fields : [(Text, Value)]; intent : Text };
  ...
  public type RemoveApprovalArgs = {
    fee : ?Nat;
    from_subaccount : ?Blob;
    spender : Blob;
  };
  ...
  public type Self = LedgerCanisterPayload -> async actor {
    ...
    get_allowances : shared query GetAllowancesArgs -> async [Allowance];
    ...
    remove_approval : shared RemoveApprovalArgs -> async Result_2;
    send_dfx : shared SendArgs -> async Nat64;
    symbol : shared query () -> async Symbol;
    tip_of_chain : shared query () -> async TipOfChainRes;
